...
install-wp:
	$(WP) core install \
		...
		--title="HuggingFace Provider Dev" \
		--admin_user=admin \
...
composer:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer install --no-interaction"
...
mozart:
	...
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer dump-autoload"

activate:
	$(WP) plugin activate huggingface-provider

test:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer test"

test-lint:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer test:lint"

test-phpstan:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer test:phpstan"

test-phpmd:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer test:phpmd"

test-docs:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/huggingface-provider && composer test:docs"
