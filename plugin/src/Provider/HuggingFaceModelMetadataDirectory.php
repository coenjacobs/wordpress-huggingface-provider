...
class HuggingFaceModelMetadataDirectory implements ModelMetadataDirectoryInterface
{
    ...
    private function getModelMetadataMap(): array
    {
        ...
        $enabledModels = get_option('huggingface_enabled_models', []);
    ...
    {
        $cached = get_transient('huggingface_models_raw');
        if ($cached !== false && is_array($cached)) {
            return $cached;
        }

        $response = wp_remote_get(HuggingFaceProvider::apiBaseUrl() . '/models', [
            'timeout' => 15,
            'headers' => ['Accept' => 'application/json'],
        ]);

        if (is_wp_error($response)) {
            set_transient('huggingface_models_fetch_error', $response->get_error_message(), HOUR_IN_SECONDS);
            return [];
        }
        ...
        if ($status !== 200) {
            set_transient(
                'huggingface_models_fetch_error',
        ...
        delete_transient('huggingface_models_fetch_error');
        set_transient('huggingface_models_raw', $models, 10 * MINUTE_IN_SECONDS);
