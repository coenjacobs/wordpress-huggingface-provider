 ...
 *
 * All Hugging Face Inference Provider models use the same OpenAI-compatible /chat/completions endpoint.
 */
class TextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    ...
    public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(
            HttpMethodEnum::POST(),
            HuggingFaceProvider::url('chat/completions'),
            $headers,
            $params,
    ...
    protected function prepareGenerateTextParams(array $prompt): array
    {
        ...
        $modelIdWithStrategy = $modelId . ':' . $strategy;

        $params = [
            'model' => $modelIdWithStrategy,
            'messages' => $messages,
        ];
